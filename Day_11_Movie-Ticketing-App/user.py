...
class User:
    ...
    def book_ticket(self, movie_name, timing, seat):
        ticket_id = str(uuid.uuid4()) # Generate a unique ticket ID 381eba1d-10af-41b3-b64f-46ed49635809
        ticket = {'movie_name': movie_name, 'timing': timing, 'seat': seat}
        self.tickets[ticket_id] = ticket
        self._save_tickets_to_csv()
        return ticket_id
    ...
    # Updated method to load movies from CSV
    def _load_tickets_from_csv(self):
        tickets = []
        with open('tickets.csv', mode='r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                tickets.append({'id': row['ID'],'movie_name': row['Movie Name'], 'timing': row['Timing'], 'seat': row['Seat']})
    ...
    # Helper method to save tickets to CSV
    def _save_tickets_to_csv(self):
        with open('tickets.csv', mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Movie Name', 'Timing', 'Seat'])
            for ticket_id, ticket in self.tickets.items():
                writer.writerow([ticket_id,ticket['movie_name'], ticket['timing'], ticket['seat']])
